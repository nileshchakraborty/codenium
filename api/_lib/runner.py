"""
Runner module for executing code in a safe environment and capturing output.
"""
import json
import subprocess
import sys
...
def run_tests_internal():
    results = []
    all_passed = True
    test_cases = """ + json.dumps(test_cases) + r"""

    # helper to find solution function
    def find_solution_func(locals_dict):
        # Filter for functions OR CLASSES defined in this module
        ignore = {
            'run_tests_internal', 'list_to_ll', 'll_to_list', 'list_to_tree',
            'tree_to_list', 'adj_to_graph', 'graph_to_adj', 'ListNode',
            'TreeNode', 'Node', 'null', 'true', 'false'
        }

        candidates = [obj for name, obj in list(locals_dict.items())
                      if (callable(obj) or inspect.isclass(obj)) and
                      obj.__module__ == '__main__' and name not in ignore]
    ...
    if not solution_func:
        print(json.dumps({
            "passed": False,
            "results": [],
            "error": "No function/class found. Define a function or class."
        }))
        return
    ...
    for i, test in enumerate(test_cases):
        ...
        try:
            ...
            # 1. Try treating as valid Python script
            # (Imperative Class Test or Semicolon Separated)
            ...
            try:
                ...
                if tree.body:
                    ...
                    if isinstance(last_node, ast.Expr):
                        # Exec prev
                        if len(tree.body) > 1:
                            prev = ast.Module(body=tree.body[:-1],
                                              type_ignores=[])
                            exec(compile(prev, filename="<string>",
                                         mode="exec"), globals(), local_scope)
                        # Eval last
                        expr = ast.Expression(body=last_node.value)
                        last_expr_val = eval(compile(expr, filename="<string>",
                                             mode="eval"),
                                             globals(), local_scope)
                    else:
            ...
            # If full_script_mode produced a result, use it directly
            # (design class imperative tests)
            if full_script_mode and last_expr_val is not None:
                ...
                cmd_var = next((k for k in local_scope
                                if 'command' in k or 'method' in k or
                                'op' in k), None)
                arg_var = next((k for k in local_scope
                                if 'arg' in k or 'input' in k or 'val' in k),
                               None)
                ...
                if is_class_solution:
                    if is_design_input:
                        ...
                        # First command is usually the class name,
                        # first arg is constructor args
                        ...
                        # Standard Algo Problem wrapped in Class
                        # (e.g. class Solution: def solve(...))
                        # 1. Instantiate (assume no-arg constructor)
                        instance = solution_func()

                        # 2. Find the method to call
                        # Heuristic: First public method that isn't __init__
                        methods = [f for f in dir(instance)
                                   if callable(getattr(instance, f)) and
                                   not f.startswith("__")]
                        if not methods:
                            raise Exception(
                                "No public methods found in class Solution"
                            )

                        # Use the first one
                        target_method = getattr(instance, methods[0])
                        ...
                        for param in sig.parameters:
                            ...
                            if found:
                                ...
                                if isinstance(val, list) and param in [
                                    'l1', 'l2', 'head', 'list1', 'list2',
                                    'headA', 'headB'
                                ]:
                                    pos = local_scope.get('pos', -1)
                                    val = list_to_ll(val, pos)
                                elif isinstance(val, list) and param in [
                                        'root', 'root1', 'root2', 'subRoot'
                                ]:
                                        val = list_to_tree(val)
                                elif isinstance(val, list) and param in ['node'] \
                    ...
                    for param in sig.parameters:
                        ...
                        if found:
                            ...
                            # If list -> convert to TreeNode
                            # (for same-tree, validate-bst)
                            # If int -> find node in tree by value
                            # (for LCA problems)
                            elif param in ['p', 'q']:
                                if isinstance(val, list):
                                    val = list_to_tree(val)
                                elif isinstance(val, int):
                                    # p/q are integer VALUES, need to find node
                                    root_tree = None
                                    for a in args:
                                        if isinstance(a, TreeNode):
                                            root_tree = a
            ...
            def convert_val(val, func_name=''):
                ...
                if isinstance(val, TreeNode):
                    # For LCA functions, return just the node value,
                    # not tree serialization
                    if 'lowestCommonAncestor' in func_name:
                        return val.val
                    return tree_to_list(val)
                if hasattr(val, 'val') and hasattr(val, 'neighbors'):
                    return graph_to_adj(val)
                # Handle Node class with random pointer
                # (copy-list-with-random-pointer)
                if hasattr(val, 'val') and hasattr(val, 'next') \
                   and hasattr(val, 'random'):
            ...
            actual_val = convert_val(result,
                                     locals().get('solution_func_name', ''))

            # Support In-Place: Only for known in-place modification functions
            # Not for functions that return new heads/roots
            in_place_funcs = ['sortColors', 'rotate', 'setZeroes',
                              'moveZeroes', 'reverseString', 'wallsAndGates',
                              'gameOfLife']

            # TODO: If Class, using in-place args is tricky unless
            # we tracked the args
            # For now, In-place on Class methods isn't explicitly requested
            # but should largely work if args refer to same objs

            if actual_val is None and result is None \
               and locals().get('args'): # Check if args exists in scope
                if locals().get('solution_func_name', '') in in_place_funcs:
                    candidates = [convert_val(a)
                                  for a in locals().get('args', [])]
                    if candidates and isinstance(candidates[0], list):
                        actual_val = candidates[0]
                else:
                    # For LinkedList/Tree functions that return
                    # None = empty result
                    actual_val = []

            if locals().get('solution_func_name') == 'groupAnagrams' \
               and isinstance(actual_val, list):
                try:
            ...
            def is_sortable_list_of_lists(lst):
                ...
                # Check sublists contain sortable homogeneous types
                try:
                    for sublist in lst:
                        if sublist and any(x is None or
                                           not isinstance(x, type(sublist[0]))
                                           for x in sublist):
                            return False
                    return True
                except:
            ...
            if is_sortable_list_of_lists(expected) \
               and is_sortable_list_of_lists(actual_val):
                try:
            ...
            if actual_val is None and isinstance(expected, list) \
               and not is_class_solution:
                actual_val = []

            # Special Handling for "k, nums = [...]" expected output
            # (removeElement, etc)
            custom_passed = None
            if isinstance(expected_str, str) and ',' in expected_str \
               and '=' in expected_str:
                 ...
                 if match:
                     try:
                         ...
                         exp_arr = eval(match.group(3), globals(),
                                        {'null': None, 'true': True,
                                         'false': False, '_': '_'})
                            ...
                            if actual_val == exp_k:
                                 ...
                                 if isinstance(modified_arr, list):
                                     ...
                                     if 'removeElement' in func_name:
                                         ...
                                         try:
                                             custom_passed = (sorted(sub_arr) ==
                                                              sorted(exp_arr_sliced))
                                         except:
                                             custom_passed = (sub_arr ==
                                                              exp_arr_sliced)
                                     else:
            ...
            if custom_passed is not None:
                ...
                # Custom test case with no expected output
                # just show result, mark as "ran"
